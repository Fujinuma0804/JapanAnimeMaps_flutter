...
// 広告視聴後に検索上限をリセットするメソッド
  Future<void> _resetSearchLimitAfterAd() async {
    print('MapSubscription: 🎁 Starting search limit reset after ad...');

    // mountedチェックを追加
    if (!mounted) {
      print('MapSubscription: ⚠️ Widget not mounted, skipping reset');
      return;
    }

    try {
      print('MapSubscription: 💾 Updating Firestore search usage...');
      ...
      if (mounted) {
        try {
          setState(() {
            ...
            _lastSearchDate = DateTime.now();
            _isWatchingAd = false;
            print('MapSubscription: ✅ Local state updated after ad reward');
          });
          ...
          SnackBar(
            content: Row(
              children: [
                ...
                SizedBox(width: 8),
                Expanded(
                  child: Text('広告視聴ありがとうございます！検索制限がリセットされました。'),
                ),
              ],
            ),
            duration: Duration(seconds: 4),
  ...
  void _showRewardedAd() {
    ...
    // mountedチェックを追加
    if (!mounted) {
      print('MapSubscription: ⚠️ Widget not mounted, skipping ad show');
      return;
    }
    ...
    if (!adAvailable) {
      print('MapSubscription: ❌ Ad not available, showing error message');
      ...
      // 広告の再読み込みを試みる
      print('MapSubscription: 🔄 Attempting to reload ad...');
      AdManager.reloadAd();
      return;
    }
    ...
    if (mounted) {
      try {
        setState(() {
          _isWatchingAd = true;
          print('MapSubscription: ✅ Set _isWatchingAd to true');
        });
      } catch (e) {
        print('MapSubscription: ❌ Error setting watching ad state: $e');
        return;
      }
    }

    print('MapSubscription: 🚀 Calling AdManager.showRewardedAd()...');

    // 広告を表示
    AdManager.showRewardedAd(() {
      print('MapSubscription: 🎁 Reward callback triggered!');
      _resetSearchLimitAfterAd();
    }).then((bool success) {
      print('MapSubscription: 📊 Ad show result: $success');

      if (!success && mounted) {
        print('MapSubscription: ❌ Ad show failed, resetting state');
        try {
          setState(() {
            _isWatchingAd = false;
          });
        } catch (e) {
          print('MapSubscription: ❌ Error updating state after ad failure: $e');
        }
      ...
      print('MapSubscription: ❌ Error in showRewardedAd: $error');
      print('MapSubscription: Stack trace: ${StackTrace.current}');
      ...
      if (mounted) {
        try {
          ...
          print('MapSubscription: ❌ Error updating state on ad error: $e');
        }
      }
    });
  }
...
// MapSubscriptionクラスの _buildSuffixIcon メソッドを以下に置き換えてください
  ...
  Widget _buildSuffixIcon() {
    ...
    if (_searchController.text.isNotEmpty) {
        ...
        child: AnimatedScale(
          ...
          child: Material(
            ...
            child: InkWell(
              borderRadius: BorderRadius.circular(20),
              onTap: () {
                print('MapSubscription: Clear button tapped');
                _searchController.clear();
                setState(() {
                  _searchResults = [];
                });
      ...
      print('MapSubscription: Search limit reached, showing ad button');
      return Container(
        padding: EdgeInsets.all(8),
        child: Material(
          color: Colors.transparent,
          child: InkWell(
            ...
            onTap: () {
              ...
              if (!_isAdAvailable) {
                print('MapSubscription: Ad not available from search bar');
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Row(
                      children: [
              ...
              // 広告を表示
              print('MapSubscription: Calling _showRewardedAd from search bar');
              _showRewardedAd();
            },
            child: AnimatedContainer(
              duration: Duration(milliseconds: 300),
    ...
    print('MapSubscription: No suffix icon needed');
              ...
              child: Row(
                children: [
                  ...
                  if (kDebugMode) ...[
                    ...
                    ElevatedButton(
                      onPressed: () {
                        print('MapSubscription: 🧪 Debug: Force reload ad');
                        AdManager.reloadAd();
                      },
