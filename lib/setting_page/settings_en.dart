...
import '../Prensantionlayer/profile_page/profile_en.dart';
import '../top_page/welcome_page.dart';
import '../bloc/Userinfo_bloc/Userinfo_bloc.dart'; // Import your BLoC
...
class _SettingsEnState extends State<SettingsEn> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<UserBloc, UserState>(
      builder: (context, state) {
        ...
        // For UserDataLoaded and UserInitialized states
        String currentLanguage = 'English';
        bool isJapanese = false;

        if (state is UserDataLoaded) {
          currentLanguage = state.language;
          isJapanese = currentLanguage == 'Japanese';
        } else if (state is UserInitialized) {
          // You might want to handle this state differently
        ...
        return WillPopScope(
          ...
          child: Scaffold(
            ...
            body: _buildSettingsList(context, isJapanese, currentLanguage),
          ),
        );
      },
    );
  ...
  Widget _buildSettingsList(
      BuildContext context, bool isJapanese, String currentLanguage) {
    return SettingsList(
      sections: [
        SettingsSection(
          title: Text(isJapanese ? '個人情報' : 'Personal Information'),
          tiles: <SettingsTile>[
            SettingsTile.navigation(
              leading: const Icon(Icons.account_circle),
              title: Text(isJapanese ? 'プロフィール' : 'Profile'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => ProfileEnScreen()),
                );
              },
            ),
            SettingsTile.navigation(
              leading: const Icon(Icons.signpost_outlined),
              title: Text(isJapanese ? '住所一覧' : 'Address List'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
        ...
        SettingsSection(
          title: Text(isJapanese ? '一般' : 'General'),
          tiles: <SettingsTile>[
            SettingsTile.navigation(
              leading: const Icon(Icons.language),
              title: Text(isJapanese ? '言語設定' : 'Language Settings'),
              value: Text(isJapanese ? '日本語' : 'English'),
              onPressed: (context) {
                _showLanguageDialog(context, isJapanese, currentLanguage);
              },
            ),
          ],
        ),
        SettingsSection(
          title: Text(isJapanese ? 'このアプリについて' : 'About This App'),
          tiles: <SettingsTile>[
            SettingsTile.navigation(
              leading: const Icon(Icons.circle_notifications),
              title: Text(isJapanese ? 'お知らせ' : 'Notice'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
            ...
            SettingsTile.navigation(
              leading: const Icon(Icons.privacy_tip_outlined),
              title: Text(isJapanese ? 'プライバシーポリシー' : 'Privacy Policy'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
            ...
            SettingsTile.navigation(
              leading: const Icon(Icons.book_outlined),
              title: Text(isJapanese ? '利用規約' : 'Terms of Service'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
            ...
            SettingsTile.navigation(
              leading: const Icon(Icons.help_outline),
              title: Text(isJapanese ? 'ヘルプセンター' : 'Help Center'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
            ...
            SettingsTile.navigation(
              leading: const Icon(Icons.info_outline_rounded),
              title: Text(isJapanese ? 'このアプリについて' : 'About This Apps'),
              value: const Text(''),
              onPressed: (context) {
                Navigator.push(
                  context,
        ...
        SettingsSection(
          tiles: <SettingsTile>[
            ...
            SettingsTile.navigation(
              ...
              title: Text(
                isJapanese ? 'アカウント削除' : 'Delete Account',
                style: TextStyle(
                  color: Colors.red,
                  fontWeight: FontWeight.bold,
                ),
              ...
              onPressed: (context) {
                _showDeleteAccountDialog(isJapanese);
              },
            ),
          ],
        ),
    ...
    showDialog(
      ...
      builder: (context) {
        return CupertinoAlertDialog(
          title: Column(
            children: [
              Text(isJapanese ? '言語設定' : 'Language Settings'),
              SizedBox(height: 8),
            ],
          ),
          content: Column(
  ...
  void _changeLanguage(String language) {
    // Use BLoC to update language
    context.read<UserBloc>().add(UpdateUserLanguage(
          language: language,
        ));
  ...
  Future<void> _showDeleteAccountDialog(bool isJapanese) async {
    final bool? shouldDelete = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text(isJapanese ? 'アカウント削除の確認' : 'Confirm Account Deletion'),
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                Text(isJapanese
          ...
          actions: <Widget>[
            ...
            TextButton(
              child: Text(isJapanese ? '削除' : 'Delete'),
              onPressed: () {
                Navigator.of(context).pop(true);
              },
            ),
    ...
    if (shouldDelete == true) {
      await _deleteAccount(isJapanese);
    }
  }
  ...
  Future<void> _deleteAccount(bool isJapanese) async {
    try {
      await _deleteUserData();
      await _deleteUserAuth();
